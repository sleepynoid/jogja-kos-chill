...
export const Route = createFileRoute("/dashboard/tambah-kos")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Tambahkan listing kos baru ke platform Keep n Sleep.",
      },
      { name: "robots", content: "noindex" },
    ],
  }),
  component: TambahKosPage,
});
...
function TambahKosPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.nama || !form.jenis || !form.daerah || !form.alamat || !form.hargaPerBulan) {
    ...
    try {
      // Get current user for folder prefix
      ...
      // 1. Upload images (convert to WebP + upload to Supabase Storage)
      toast.info("Mengupload foto...");
      const files = gambar.map((g) => g.file);
      const urls = await uploadKosImages(files, user.uuid);

      // 2. Save kos to database via server function
      toast.info("Menyimpan data kos...");
      const result = await createKosFn({
        data: {
          nama: form.nama,
          jenis: form.jenis,
      ...
      const message = err instanceof Error ? err.message : "Gagal menambahkan kos.";
      toast.error(message);
    } finally {
      setLoading(false);
    }
      ...
      >
          ...
          <div className="grid gap-5 md:grid-cols-2">
            <div className="md:col-span-2">
              ...
              <input
                ...
                placeholder="Contoh: Griya Sogan Bulaksumur"
            ...
            <div className="md:col-span-2">
              ...
              <input
                ...
                placeholder="Jl. Kaliurang KM 4.5, Sleman"
                className="w-full rounded-xl border border-input bg-background px-4 py-2.5 text-sm outline-none transition-all focus:ring-2 focus:ring-ring"
              />
            </div>
        ...
        <section>
          ...
          <p className="text-xs text-muted-foreground mb-3">
            Pilih satu atau lebih kampus yang dekat dengan kos Anda.
          </p>
        ...
        <section>
          ...
          <p className="text-xs text-muted-foreground mb-3">
            Unggah foto kos (maks {MAX_FILES} foto, 5MB per foto). Foto pertama akan jadi cover.
          </p>
